...
import (
	base3 "CipT/Core/NoKey/BaseFamily/codec/base"
)
...
func NewCodec(encoder string) (base3.IEncoding, error) {
	if len(encoder) != stdEncoderSize {
		return nil, base3.ErrEncoderSize(codec, stdEncoderSize)
	}
	if encoder[0] == encoder[1] {
		return nil, base3.ErrEncoderRepeatChar(codec)
	}
	b := &base2Codec{decodeMap: make(map[byte]int, stdEncoderSize)}
	for k, v := range encoder {
		if base3.IsIllegalCharacter(v) {
			return nil, base3.ErrEncoderIllegalChar(codec)
		}
...
func (b *base2Codec) decode(dst, src []byte) (int, error) {
	...
	for k, v := range src {
		...
		if !ok {
			return 0, base3.ErrEncodedText(codec, v, k)
		}
...
func (b *base2Codec) Decode(src []byte) ([]byte, error) {
	...
	if size%stdBlockLen != 0 {
		return nil, base3.ErrEncodedTextSize(codec, size, stdBlockLen)
	}
