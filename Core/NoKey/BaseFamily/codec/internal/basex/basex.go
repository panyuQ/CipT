...
func NewBasex(name, encoder string, encodeSize int) (*Basex, error) {
	if len(encoder) != encodeSize {
		return nil, base2.ErrEncoderSize(name, encodeSize)
	}
	...
	for k, v := range encoder {
		if base2.IsIllegalCharacter(v) {
			return nil, base2.ErrEncoderIllegalChar(name)
		}
	...
	if len(mp) != encodeSize {
		return nil, base2.ErrEncoderRepeatChar(name)
	}
	return b, nil
}
...
func (b *Basex) Decode(src []byte) ([]byte, error) {
	...
	for k, v := range src {
		...
		if !ok {
			return nil, base2.ErrEncodedText(b.name, v, k)
		}
